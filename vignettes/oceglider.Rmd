...
In the above, the UNESCO equation of state is used, because the newer TEOS10/GSW
version requires longitude and latitude information to be valid, which is not
the case in the present version of the package at the end of a dataset, owing to
how [`read.glider.seaexplorer.delayed()`]
(../help/read.glider.seaexplorer.delayed) works.

There is a problem with the graph just constructed, with a spurious-looking line
connecting near-surface and deep data. To learn more about that, we may use an
\CRANpkg{oce} function to create a "scan" plot, say of the last of these `ctd`
objects just created.
```{r}
plotScan(ctd, type="p")
```

The scan plot makes it clear that the yo contains near-surface data, then skips
to depth, and then returns to the surface. A more careful examination with
```{r}
par(mfrow=c(1, 2))
