...
e.g.
```{r}
g <- read.glider.seaexplorer.realtime(dir)
```
...
`"sub"` in the filenames indicates that these real-time data transmitted during
the mission are actually just subsets of the full data being acquired.

Files with `"raw"` in the filename hold the fuller datasets that are downloaded
from the glider after recovery. In the oceanographic convention, these are
called delayed-mode data, and so the function to read them is called
