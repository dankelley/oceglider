...
plot(g2, which="map")
```
Notice that some of the spots seem to have only a few data. This can be checked with
```{r}
table(g2[["yoNumber"]])
```
which shows that yoNumber 101, 103 and 105 contain too few data to be considered profiles. There are
several ways to explore this further, but for the present purpose, let's make a new object
that contains only those yos that have more than, say, 5 points:
```{r}
g3 <- subset(g2, levels >= 5)
```
As a check that,
```{r}
unique(g3[["yoNumber"]])
```
verifies that we got the yos we want.
...
par(mfrow=c(1, 3)) # we happen to know there are 3 significant yos
for (ss in s) {
   ...
   oce::plotProfile(ctd, which="temperature", eos="unesco")
}
```
...
Note how there is a line connecting near-surface and deep data. To learn more about that, let's use
an oce function to create a ``scan'' plot, say with the last of these `ctd` objects just created.
```{r}
plotScan(ctd, type="p")
```
...
```{r}
par(mfrow=c(1, 3))
plot(s[[1]][["time"]], s[[1]][["pressure"]])
plot(s[[1]][["time"]], s[[1]][["navState"]])
```
shows us that if the desire was to obtain a simple upcast, we might consider
...
it may be sensible to select for 117, if the desire is to focuss on the ascent
...
This looks reasonable now. Let's try a colour-coded TS plot, black
for the first of these yos, red for the second, and green for the
third.
```{r}
par(mfrow=c(1, 1))
for (i in seq_along(s)) {
    ...
    ctd <- oce::ctdTrim(ctd, "upcast")
    if (i == 1) {
       oce::plotTS(ctd, eos="unesco")
    } else {
       points(ctd[["salinity"]], ctd[["temperature"]], col=i)
    }
}
````
