...
table(g2[["yoNumber"]])
```
which shows that some `yoNumber` values contain too few data to be considered
profiles. There are several ways to explore this further, but for the present
purpose, let's make a new object that contains only those yos that have more
than, say, 5 points:
```{r}
