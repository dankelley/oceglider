...
```{r eval=FALSE}
identical(is.na(g[["O2"]]), is.na(g[["pressure"]]))
identical(is.na(g[["O2"]]), is.na(g[["time"]]))
```
so we can construct a time-depth plot colour-coded for oxygen with
```{r eval=FALSE}
O2 <- g[["O2"]]
t <- g[["time"]]
p <- g[["pressure"]]
# Construct Figure 2
par(mar=c(2, 3.3, 1, 1))
cm <- colormap(O2, col=oceColorsTurbo)
drawPalette(colormap=cm)
oce.plot.ts(time, pressure, ylim=rev(range(pressure, na.rm=TRUE)),
    ylab=resizableLabel("pressure"),
...
![Figure 1. Glider surfaced locations (red dots) with 500m depth contour (black line)](slocum_map.png)

![Figure 2. Depth dependence of O$_2$ as sampled by the glider whose path southward and then northward is illustrated in Figure 1.](slocum_tz.png)
